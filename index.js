...
if (!tema) {
    console.log('passe um tema. Ex: node index.js tecnologia');
    process.exit();
}
...
async function extrairNoticiasDaPagina(page) {
    await page.waitForSelector('li[data-testid="search-bodega-result"]');

    const noticias = await page.$$eval('li[data-testid="search-bodega-result"]', (itens) => {
        return itens.map((item) => {
            const titulo = item.querySelector('h4')?.innerText?.trim() || '';

            const paragrafos = Array.from(item.querySelectorAll('p'))
                .map(p => p.innerText.trim())
                .filter(texto => texto.length > 30 && !texto.startsWith('By'));
...
async function main() {
    ...
    while (qtdResultados < 50) {
        const seletorBotao = '[data-testid="search-show-more-button"]';
        const botaoExiste = await page.$(seletorBotao);

        if (!botaoExiste) break;

        await page.$eval(seletorBotao, el => el.scrollIntoView({ block: 'center' }));
        await new Promise(resolve => setTimeout(resolve, 1000));

        await page.$eval(seletorBotao, el => el.click());

        try {
            await page.waitForFunction(
                (qtdAnterior) =>
                    document.querySelectorAll('li[data-testid="search-bodega-result"]').length > qtdAnterior,
                { timeout: 8000 },
                qtdResultados
            );
        } catch (e) {
    ...
    const noticiasParaExcel = todasNoticias.slice(0, 50);

    if (noticiasParaExcel.length < 50) {
